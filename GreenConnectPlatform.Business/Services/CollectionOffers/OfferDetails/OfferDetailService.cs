...
public class OfferDetailService : IOfferDetailService
{
    ...
    public async Task<OfferDetailModel> AddOfferDetail(Guid collectorId,Guid collectionOfferId ,OfferDetailCreateModel offerDetailCreateModel)
    {
        ...
        if(collectionOffer.ScrapCollectorId != collectorId)
            throw new ApiExceptionModel(StatusCodes.Status403Forbidden, "403", "You can not add offer detail to this collection offer");
        if(collectionOffer.Status != OfferStatus.Rejected && collectionOffer.Status != OfferStatus.Canceled)
            throw new ApiExceptionModel(StatusCodes.Status400BadRequest, "400", "You can only add offer details to reject or cancel collection offers");
        ...
        if (!allCategoriesIdInPost.Contains(offerDetailCreateModel.ScrapCategoryId))
            throw new ApiExceptionModel(StatusCodes.Status400BadRequest, "400", "Scrap category does not belong to the original scrap post.");
        ...
        if (detailAlreadyExists)
            throw new ApiExceptionModel(StatusCodes.Status409Conflict, "409", "An offer detail for this scrap category already exists in the collection offer.");
    ...
    public async Task<OfferDetailModel> UpdateOfferDetail(Guid collectorId, Guid offerDetailId, OfferDetailUpdateModel offerDetailUpdateModel)
    {
        var offerDetail = await _offerDetailRepository.DbSet()
        ...
        if(offerDetail.CollectionOffer.Status == OfferStatus.Accepted)
            throw new ApiExceptionModel(StatusCodes.Status400BadRequest, "400", "You can not update offer details of accepted collection offers");
    ...
    public async Task DeleteOfferDetail(Guid collectorId, Guid offerDetailId)
    {
        ...
        if(offerDetail.CollectionOffer.ScrapCollectorId != collectorId)
            throw new ApiExceptionModel(StatusCodes.Status403Forbidden, "403", "You can not update this offer detail");
        if(offerDetail.CollectionOffer.Status != OfferStatus.Canceled && offerDetail.CollectionOffer.Status != OfferStatus.Rejected)
            throw new ApiExceptionModel(StatusCodes.Status400BadRequest, "400", "You can only update offer details of cancel or rejected collection offers");
