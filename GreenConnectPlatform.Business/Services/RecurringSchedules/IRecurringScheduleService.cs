...
{
    Task<PaginatedResult<RecurringScheduleOverallModel>> GetPagedRecurringSchedulesAsync(int pageNumber, int pageSize, bool sortByCreatedAt);
    ...
    Task<RecurringScheduleDetailModel> AddRecurringScheduleDetailAsync(Guid recurringScheduleId, Guid userId, RecurringScheduleDetailCreateModel model);
    Task<RecurringScheduleDetailModel> UpdateRecurringScheduleDetailAsync(Guid recurringScheduleId, Guid id, Guid userId, double? quantity, string? amountDescription, ItemTransactionType? type);
