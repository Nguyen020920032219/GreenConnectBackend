...
public class TransactionDetailService : ITransactionDetailService
{
    ...
    public async Task<List<TransactionDetailModel>> CreateTransactionDetails(Guid transactionId, Guid scrapCollectorId,
        List<TransactionDetailCreateModel> transactionDetailCreateModels)
    {
        ...
        if (transaction.ScrapCollectorId != scrapCollectorId)
            throw new ApiExceptionModel(StatusCodes.Status403Forbidden, "403",
                "You can not add details to this transaction");
        if (transaction.Status == TransactionStatus.Completed)
            throw new ApiExceptionModel(StatusCodes.Status400BadRequest, "400",
                "Cannot add details to a completed transaction");
        if (transaction.CheckInTime == null)
            throw new ApiExceptionModel(StatusCodes.Status400BadRequest, "400",
                "Cannot add details to a transaction that has not been checked in");
        ...
        if (categoryInModels.Count != distinctCategoryIds.Count)
            throw new ApiExceptionModel(StatusCodes.Status409Conflict, "409",
                "Duplicate scrap category in transaction details");
        foreach (var detail in transactionDetailCreateModels)
        {
            var exists = categoryInScrapPost.Contains(detail.ScrapCategoryId);
            if (!exists)
                throw new ApiExceptionModel(StatusCodes.Status400BadRequest, "400",
                    $"Scrap category with ID {detail.ScrapCategoryId} does not exist in the related scrap post");
        }
        ...
        if (!categoryInOfferDetails.SetEquals(categoryInModels))
            throw new ApiExceptionModel(StatusCodes.Status400BadRequest, "400",
                "Transaction details do not match the accepted offer.");
    ...
    public async Task<TransactionDetailModel> UpdateTransactionDetail(Guid scrapCollectorId, Guid transactionId,
        int scrapCategoryId,
        TransactionDetailUpdateModel transactionDetailUpdateModel)
    {
        ...
        if (transactionDetailUpdateModel.PricePerUnit == null)
            transactionDetailUpdateModel.PricePerUnit = transactionDetail.PricePerUnit;
        if (transactionDetailUpdateModel.Quantity == null)
            transactionDetailUpdateModel.Quantity = transactionDetail.Quantity;
