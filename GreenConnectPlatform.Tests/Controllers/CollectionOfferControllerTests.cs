...
public class CollectionOfferControllerTests
{
    ...
    public async Task BOK01_GetMyOffers_ReturnsOk_WithPaginatedData()
    {
        ...
        var pagedResult = new PaginatedResult<CollectionOfferOveralForCollectorModel>
        {
            ...
            {
                new() { CollectionOfferId = Guid.NewGuid(), Status = OfferStatus.Pending }
            },
        ...
        // Act
        var result = await _controller.GetMyOffers(OfferStatus.Pending);

        // Assert
        var okResult = result.Should().BeOfType<OkObjectResult>().Subject;
        var returnData = okResult.Value.Should().BeOfType<PaginatedResult<CollectionOfferOveralForCollectorModel>>()
            .Subject;
    ...
    public async Task BOK11_UpdateDetail_ReturnsOk_WhenValid()
    {
        ...
        _mockOfferService.Setup(s => s.UpdateDetailAsync(_testUserId, offerId, detailId, request))
    ...
    public async Task BOK19_GetTransactions_ReturnsOk_WithData()
    {
        ...
        {
            Data = new List<TransactionOveralModel> { new() { TransactionId = Guid.NewGuid() } },
            Pagination = new PaginationModel(1, 1, 10)
        };
